const luvicii = {
  ...
  snackbarShow: (text, showActionFunction = false, duration = 2000, actionText = false) => {
    ...
    root.style.setProperty("--luvicii-snackbar-time", duration + "ms");
  ...
  initThemeColor: function () {
    let themeColor = getComputedStyle(document.documentElement)
      .getPropertyValue("--luvicii-bar-background")
      .trim()
      .replace('"', "")
      .replace('"', "");
    ...
    if (currentTop > 26) {
      if (luvicii.is_Post()) {
        themeColor = getComputedStyle(document.documentElement)
          .getPropertyValue("--luvicii-meta-theme-post-color")
          .trim()
          .replace('"', "")
          .replace('"', "");
      }
  ...
  downloadImage: function (imgsrc, name) {
    ...
    if (rm.downloadimging == false) {
      ...
      luvicii.snackbarShow("正在下载中，请稍后", false, 10000);
      setTimeout(function () {
        let image = new Image();
        // 解决跨域 Canvas 污染问题
        ...
        image.src = imgsrc;
        luvicii.snackbarShow("图片已添加盲水印，请遵守版权协议");
        rm.downloadimging = false;
      }, "10000");
    } else {
      luvicii.snackbarShow("有正在进行中的下载，请稍后再试");
    }
  },
  //禁止图片右键单击
  ...
  catalogActive: function () {
    ...
    if ($list) {
      ...
      luvicii.scrollByMouseWheel($list, $catalog);
    }
  },
  // Page Tag 激活
  tagsPageActive: function () {
    ...
    if ($list) {
      ...
      luvicii.scrollByMouseWheel($list, $tagPageTags);
    }
  },
  // 修改时间显示"最近"
  ...
  changeTimeInEssay: function () {
    document.querySelector("#bber") &&
      document.querySelectorAll("#bber time").forEach(function (e) {
        ...
        (t.innerText = luvicii.diffDate(datetime, true)), (t.style.display = "inline");
      });
  },
  // 修改相册集中的时间
  ...
  musicToggle: function (changePaly = true) {
    if (!luvicii_musicFirst) {
      luvicii.musicBindEvent();
      luvicii_musicFirst = true;
    }
  ...
  initEssayLrcStage: function () {
    document.querySelectorAll("#bber .bber-music meting-js").forEach(metingEl => {
      ...
      const boot = setInterval(() => {
        ...
        if (tryBuild()) return;
        // preload=none 时歌词可能在首次播放后才加载
        player.on("play", () => {
          let tries = 0;
          const poll = setInterval(() => {
            if (tryBuild() || ++tries > 50) clearInterval(poll);
          }, 200);
        });
      }, 200);
  ...
  addEventListenerMusic: function () {
    ...
    // 以下页面级与全局事件只需绑定一次，切换歌单重建播放器后不再重复绑定
    if (anMusicPage.dataset.eventsBound) return;
    anMusicPage.dataset.eventsBound = "true";
    ...
    anMusicBtnGetSong.addEventListener("click", () => {
      const metingEl = document.querySelector("#anMusic-page meting-js");
      const player = metingEl && metingEl.aplayer;
      if (!player) return;
      const audios = player.list.audios;
      if (!audios.length) return;
      player.list.switch(Math.floor(Math.random() * audios.length));
      player.play();
    });
    // 重新拉取当前歌单
    ...
    // 全局事件（menu-mask、键盘）只绑定一次，播放器实时获取，兼容切换歌单与 pjax
    if (luvicii.musicGlobalEventsBound) return;
    luvicii.musicGlobalEventsBound = true;

    function anMusicPageMenuAask() {
      if (window.location.pathname != "/music/") return;
      const aplayerList = document.querySelector("#anMusic-page .aplayer-list");
      if (aplayerList) aplayerList.classList.remove("aplayer-list-hide");
    }
    ...
    document.addEventListener("keydown", function (event) {
      ...
      if (!musicPage) return;
      const metingEl = musicPage.querySelector("meting-js");
      const player = metingEl && metingEl.aplayer;
      if (!player) return;
      //暂停开启音乐
      if (event.code === "Space") {
  ...
  musicBindEvent: function () {
    document.querySelector("#nav-music .aplayer-music").addEventListener("click", function () {
      luvicii.musicTelescopic();
    });
    document.querySelector("#nav-music .aplayer-button").addEventListener("click", function () {
      luvicii.musicToggle(false);
    });
  },
  ...
  keyboardToggle: function () {
    const isKeyboardOn = luvicii_keyboard;

    if (isKeyboardOn) {
      const consoleKeyboard = document.querySelector("#consoleKeyboard");
      consoleKeyboard.classList.remove("on");
      luvicii_keyboard = false;
    } else {
      const consoleKeyboard = document.querySelector("#consoleKeyboard");
  ...
  scrollCategoryBarToRight: function () {
    ...
    if (items && nextButton) {
      ...
      if (items.scrollLeft + items.clientWidth + 1 >= items.scrollWidth) {
        ...
        nextButton.innerHTML = '<i class="luviciifont luvicii-icon-angle-double-right"></i>';
      } else {
        // 滚动到下一个视图
        items.scrollBy({
          left: itemsWidth,
