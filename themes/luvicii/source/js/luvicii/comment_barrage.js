  ...
  function popCommentBarrage(data) {
    ...
    barrage.innerHTML = `
          <div class="barrageHead">
            ...
            }" href="javascript:luvicii.scrollTo('#post-comment')"">
            ...
            <a class="comment-barrage-close" href="javascript:luvicii.switchCommentBarrage()"><i class="luviciifont luvicii-icon-xmark"></i></a>
          </div>
          <luvicii class="barrageContent" onClick="window.location.hash = '${data.id}'">
            ${data.comment}
          </luvicii>
        `;

    // 获取luvicii标签内的所有pre元素
    let luviciiPres = barrage.querySelectorAll("luvicii pre");

    // 遍历每个pre元素，将其替换为"【代码】"
    luviciiPres.forEach(pre => {
    ...
    // 获取luvicii标签内的所有图片元素
    let luviciiImages = barrage.querySelectorAll("luvicii img");

    // 遍历每个图片元素，将其替换为"【图片】"，但排除带有class=tk-owo-emotion的图片
    luviciiImages.forEach(image => {
