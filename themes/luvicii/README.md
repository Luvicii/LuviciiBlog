...
预览: 👍 [Luvicii](https://blog.anheyu.com/) || 🤞 [Luvicii](https://index.anheyu.com/)

文档: 📖 [luvicii Docs](https://docs.anheyu.com/)
...
修改 hexo 配置文件`_config.yml`，把主题改为`luvicii`
...
覆盖配置可以使`主题配置`放置在 luvicii 目录之外，避免在更新主题时丢失自定义的配置。
...
- windows
  复制`/themes/luvicii/_config.yml`此文件到 hexo 根目录，并重命名为`_config.luvicii.yml`

以后如果修改任何主题配置，都只需修改 _config.luvicii.yml 的配置即可。

注意：
 - 只要存在于 `_config.luvicii.yml` 的配置都是高优先级，修改原 `_config.yml` 是无效的。
 - 每次更新主题可能存在配置变更，请注意更新说明，可能需要手动对 `_config.luvicii.yml` 同步修改。
