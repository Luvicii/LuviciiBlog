...
Preview: 👍 [Luvicii](https://blog.anheyu.com/) || 🤞 [Luvicii](https://index.anheyu.com/)

Document: 📖 [luvicii Docs](https://docs.anheyu.com/)
...
```powershell
git clone -b main https://github.com/anzhiyu-c/hexo-theme-anzhiyu.git themes/luvicii
```

## ⚙ 应用主题

Modify the hexo configuration file `_ config.yml` and change the theme to `luvicii`
...
Override configuration allows the `theme configuration `to be placed outside the luvicii directory to avoid losing custom configurations when updating the theme.
...
- macos/linux
  ...
  ```bash
  cp -rf ./themes/luvicii/_config.yml ./_config.luvicii.yml
  ```

- windows
  Copy the file `/ themes/luvicii/_ config.yml` to the hexo root directory and rename it to` _ config.luvicii.yml`

If you modify any theme configuration in the future, you only need to modify the configuration of_config.luvicii.yml.

Note:
  - as long as the configurations that exist in `_ config.luvicii.yml` are of high priority, it is invalid to modify the original` _ config.yml`.
  - there may be configuration changes each time the topic is updated. Please pay attention to the update instructions. You may need to manually modify `_ config.luvicii.yml`.
